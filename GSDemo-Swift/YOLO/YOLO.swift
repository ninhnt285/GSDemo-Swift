...
//  let model = TinyYOLO()
  ...
  public func predict(image: CVPixelBuffer) throws -> [Prediction] {
    if let output = try? model.prediction(image: image, iouThreshold: Double(iouThreshold), confidenceThreshold: confidenceThreshold) {
  ...
  public func computeBoundingBoxes(features: yolov7Output) -> [Prediction] {
//    assert(features.count == 125*13*13)

      var predictions = [Prediction]()
        let confidence = features.confidence
        let coords = features.coordinates
